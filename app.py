...
def pagina_inicial():
    ...
    with col2:
        st.subheader("Faturas de Cartão de Crédito em Aberto")
        ...
        df_faturas_futuras = df_copy[
            (df_copy['Forma de Pagamento'].isin(cartoes)) &
            (
                (df_copy['Ano'] > ano_atual) |
                ((df_copy['Ano'] == ano_atual) & (df_copy['Mes_Num'] > mes_atual_num))
            )
        ]

        if not df_faturas_futuras.empty:
            faturas_em_aberto = df_faturas_futuras.groupby(['Ano', 'Mês', 'Forma de Pagamento'])['Valor'].sum().reset_index()
            faturas_em_aberto['Mes_Num'] = faturas_em_aberto['Mês'].map(MAPA_MESES_REVERSO)
            faturas_em_aberto.sort_values(by=['Ano', 'Mes_Num'], inplace=True)
            faturas_em_aberto.drop(columns='Mes_Num', inplace=True)
            st.dataframe(faturas_em_aberto, hide_index=True)
        else:
            st.info("Nenhuma fatura futura encontrada.")
