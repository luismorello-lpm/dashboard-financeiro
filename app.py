...
def pagina_adicionar():
    ...
    with st.form("novo_lancamento_form"):
        ...
        c1, c2, c3 = st.columns(3)
        dt = c1.date_input("Data do Lançamento", datetime.now())
        ...
        c1, c2 = st.columns(2)
        cl = c1.selectbox("Classificação", CLASSIFICACOES_PADRAO)
        mt = c2.selectbox("Método de Pagamento", MEIOS_PAGAMENTO_PADRAO)
        fr = c1.selectbox("Forma de Pagamento", ['À Vista', 'Parcelado'])
        pg = c2.selectbox("Já foi pago?", ['Sim', 'Não'])
        obs = st.text_area("Observações", "-")

        st.markdown("---")
        st.subheader("Configuração de Parcelas / Mês Contábil")
        cp1, cp2, cp3 = st.columns(3)
        n_p = cp1.number_input("Nº parcelas (Se parcelado)", min_value=1, value=1, step=1)
        m_c = cp2.number_input("Mês Inicial / Contábil", 1, 12, dt.month)
        a_c = cp3.number_input("Ano Inicial / Contábil", 2024, 2030, dt.year)

        submitted = st.form_submit_button("Adicionar Lançamento")

        if submitted:
            novos_lancamentos = []
            pago_str = 'OK' if pg == 'Sim' else 'NOK'
            
            # --- SE FOR À VISTA OU RECEITA ---
            if fr == 'À Vista' or tipo != 'Despesa':
                novo = {
                    'ID': proximo_id, 'Data': dt.strftime('%d/%m/%Y'), 'Tipo': tipo, 
                    'Descrição': desc, 'Valor': valor, 'Categoria': cat, 
                    'Classificação': cl if tipo == 'Despesa' else 'N/A', 
                    'Pagamento': fr if tipo == 'Despesa' else 'N/A', 
                    'Forma de Pagamento': mt if tipo == 'Despesa' else 'N/A', 
                    'Parcelas': 'N/A', 'Pagamento Realizado': pago_str, 
                    'Observações': obs, 'Mês': MAPA_MESES[m_c], 'Ano': a_c
                }
            ...
            # --- SE FOR PARCELADO ---
                ...
                for i in range(int(n_p)):
                    ...
                    novo = {
                        ...
                        'Tipo': tipo, 
                        'Descrição': f"{desc} ({i+1}/{int(n_p)})", 
                        'Valor': v_p, 
                        'Categoria': cat, 
                        'Classificação': cl, 
                        'Pagamento': fr, 
                        'Forma de Pagamento': mt, 
                        'Parcelas': f"{i+1:02d} de {int(n_p):02d}", 
                        'Pagamento Realizado': pago_str, 
                        'Observações': obs, 
                        'Mês': MAPA_MESES[m_atual], 'Ano': a_atual
                    }
                    novos_lancamentos.append(novo)

            # Salva o bloco inteiro de uma vez
            df_final = pd.concat([df_existente, pd.DataFrame(novos_lancamentos)], ignore_index=True)
            if salvar_dados(df_final):
                st.success(f"Sucesso! {len(novos_lancamentos)} registro(s) criado(s).")
                st.rerun()
