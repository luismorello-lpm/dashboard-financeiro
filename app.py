...
def carregar_dados():
    ...
    if client:
        try:
            ...
            # Garante que a coluna Mês está na ordem correta
    ...
    if client:
        try:
            ...
            if 'Valor' in df_para_salvar.columns:
                 df_para_salvar['Valor'] = df_para_salvar['Valor'].apply(lambda x: str(round(x, 2)).replace('.', ',') if pd.notna(x) else '')
...
def pagina_graficos():
    ...
    escolha = st.selectbox("Escolha o Gráfico", list(opcoes_graficos.keys()))
    
    tipo_grafico = opcoes_graficos[escolha]
